...
function StudentAccount() {
  ...
  if (error) return "Error loading Student Accounts";

  if (loading) return <h1 className="text-center my-4">Fetching...</h1>;
  if (data.users && data.users.length) {
    var searchQuery = data.users.filter(
      (query) => query.role.name == "Student"
    );

    if (searchQuery.length != 0) {
      return (
        <>
          <h1 className="h2">Student Account</h1>
          <hr />
      ...
      return <h1 className="text-center my-4">No Student Account Found</h1>;
    }
  }
  return (
    <h5 className="text-center my-4">No Student Account Found! Add Account</h5>
  );
}
